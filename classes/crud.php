...
// Inserting
		...
		public function addDiagnosis($value)
	{
		...
		$ulcer= $this->cleanse($post['ulcer']);
		$preg = $this->cleanse($post['preg']);
		$result = $this->cleanse($post['result']);
		$advice = $this->cleanse($post['advice']);

		$query="INSERT INTO diagnosis(age,f1,f2,f3,f4,f5,f6,f7,f8,ulcer,preg,result,advice) VALUES ('$age','$f1','$f2','$f3','$f4','$f5','$f6','$f7','$f8','$ulcer','$preg','$result','$advice')";
		$sql = $this->con->query($query);
		if ($sql==true) {
		...
		public function compareDiagnosis($post)
	{
		...
		$query = "SELECT * from diagnosis where age='$age' and f1='$f1' and f2='$f2' and f3='$f3' and f4='$f4' and f5='$f5' and f6='$f6' and f7='$f7' and f8='$f8' and ulcer='$ulcer' and preg='$preg' ";
		$result = $this->con->query($query);
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();
			return $row;
